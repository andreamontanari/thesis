...
import sweepline.ConflictGraph;
...
public class Aggregation {
	...
	public static Node getMaxDegNode() {	// -- O(N) con N numero di elementi di ANS
		Node n = null;
		n = ANS.get(0);
	...
	public static List<Element> minAggregation(double gt, int rt) {
		...
		{ 
			...
			Node n = getMaxDegNode(); //recupero il nodo con grado di sovrapposizione massimo  -- O(N)
	...
	public static void secondPartA(int numIcon, double gradeT, int relevance) {
		initSet(numIcon);
		minAggregation(gradeT, relevance);
	}
